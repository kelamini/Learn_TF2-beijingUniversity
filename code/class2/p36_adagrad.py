...
# 使用seed 使每次生成的随机数相同（方便教学，使大家结果都一致，在现实使用时不写 seed）
...
epoch = 500  # 循环 500 轮
loss_all = 0  # 每轮分 4 个 step，loss_all 记录 4 个 step 生成的 4 个 loss 的和
...
for epoch in range(epoch):  # 数据集级别的循环，每个 epoch 循环一次数据集
    for step, (x_train, y_train) in enumerate(train_db):  # batch 级别的循环 ，每个 step 循环一个 batch
        with tf.GradientTape() as tape:  # with 结构记录梯度信息
            ...
            y_ = tf.one_hot(y_train, depth=3)  # 将标签值转换为独热码格式，方便计算 loss和 accuracy
...
# 请将 loss 曲线、ACC 曲线、total_time 记录到  优化器对比.docx  对比各优化器收敛情况
